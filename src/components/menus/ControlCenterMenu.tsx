...
const SliderComponent = ({ iconNode, value, setValue }: SliderProps) => (
        ...
        {isMobile ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", height: "160px" }}>
              ...
              <div style={{ width: "100%", height: "160px", background: "rgba(50,40,70,0.9)", borderRadius: "20px", padding: "16px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  {playing ? (
                    <img style={{ width: "40px", height: "40px", borderRadius: "6px", boxShadow: "0 1px 3px rgba(0,0,0,0.3)", objectFit: "cover" }} src={currentTrack?.cover || music.cover} alt="cover" />
                  ) : (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    {playing ? (
                      <>
                        <span style={{ fontSize: "14px", fontWeight: 600, color: "white", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{currentTrack?.title || music.title}</span>
                        <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.8)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{currentTrack?.artist || music.artist}</span>
                      </>
                    ) : (
          ...
          <>
            ...
            <div className="cc-grid col-span-4 px-2.5 py-2 space-y-1 flex flex-col justify-around">
              ...
              <SliderComponent iconNode={<span className="i-ph:sun text-xs text-c-500" />} value={brightness} setValue={setBrightness} />
            </div>

            <div className="cc-grid col-span-4 px-2.5 py-2 space-y-1 flex flex-col justify-around">
              <span className="font-medium ml-0.5" style={{ fontSize: '12px' }}>Sound</span>
              <SliderComponent iconNode={<span className="i-ph:speaker-high text-xs text-c-500" />} value={volume} setValue={setVolume} />
            </div>

            <div className="player cc-grid col-span-4 hstack space-x-2.5" p="y-2 l-2 r-4">
              <img
                className="w-12 rounded-lg"
                src={currentTrack?.cover || music.cover}
                alt="cover art"
                style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}
              />
              <div className="flex-1">
                <div className="font-medium" style={{ fontSize: '12px' }}>{currentTrack?.title || music.title}</div>
                <div className="cc-text">{currentTrack?.artist || music.artist}</div>
              </div>
              {playing ? (
