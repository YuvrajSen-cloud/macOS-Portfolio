...
export default function Spotlight({
  ...
  const renderPortfolio = () => {
    const mainApps = apps.filter((a) => !a.hideFromDock);
    const chunks = [];
    for (let i = 0; i < mainApps.length; i += 4) {
      chunks.push(mainApps.slice(i, i + 4));
    }
    const categoryNames = ["Favorites", "System", "Productivity", "Media"];

    return (
      <div className="w-full h-full bg-transparent overflow-y-scroll p-4 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-6 content-start">
        {chunks.map((chunk, chunkIndex) => (
          <div key={chunkIndex} className="flex flex-col">
            <span className="text-[13px] text-c-600 font-medium ml-3 mb-1.5">{categoryNames[chunkIndex] || `Apps ${chunkIndex + 1}`}</span>
            <div className="bg-gray-200/50 dark:bg-gray-800/50 rounded-[24px] p-3.5 grid grid-cols-2 gap-3 backdrop-blur-md shadow-sm">
              {Array.from({ length: 4 }).map((_, index) => {
