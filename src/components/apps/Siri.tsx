...
//  Yuvraj resume context for conversational answers 
const YUVRAJ_INFO = `Yuvraj Sen is a B.Tech Computer Science & Engineering student and an aspiring Software Engineer.
He is passionate about backend engineering, especially building with Ruby on Rails, crafting efficient APIs, and managing robust database workflows.
He is continually learning modern backend architectures, system design, and exploring new web technologies.
Contact: Email senyuvaj997@gmail.com | GitHub @yuvrajsen-cloud | LinkedIn: yuvraj-sen-772434334
His resume is available for download at /resume.pdf.`;

const SIRI_FALLBACK = "Hey, I appreciate the curiosity! But I can only perform actions that Yuvraj has set up for me. He built me to help navigate his portfolio — try asking me to open an app, play music, toggle dark mode, or check the time!";

const SYSTEM_PROMPT = `You are Siri, a friendly and chill virtual assistant running inside Yuvraj Sen's macOS-style web portfolio.
...
Here is info about Yuvraj (use this when someone asks about him or his resume):
${YUVRAJ_INFO}

IMPORTANT RULES:
1. If you don't understand the request, or it is unrelated to the portfolio, reply EXACTLY: "${SIRI_FALLBACK}"
2. If asked about Yuvraj, his skills, experience, resume, or background — ALWAYS call the download_resume tool AND in your reply, share a brief friendly summary about Yuvraj (mention he's a B.Tech CS student, skilled backend developer with Ruby on Rails, APIs, and databases). Make it sound like you're genuinely proud of him!
...
6. You can open the launchpad to show all apps, open Spotify to play music, toggle fullscreen, and more.
7. Never say "I am Yuvraj" — you are Siri, built BY Yuvraj.`;
...
const TOOLS = [
  ...
  { type: "function", function: { name: "open_app", description: "Opens an app window.", parameters: { type: "object", properties: { app_id: { type: "string", description: "One of: safari, spotify, bear, terminal, vscode, facetime, typora, photos, music, maps, messages, calculator, notes" } }, required: ["app_id"] } } },
  { type: "function", function: { name: "close_app", description: "Closes an app window.", parameters: { type: "object", properties: { app_id: { type: "string", description: "One of: safari, spotify, bear, terminal, vscode, facetime, typora, photos, music, maps, messages, calculator, notes" } }, required: ["app_id"] } } },
  ...
  { type: "function", function: { name: "download_resume", description: "Downloads Yuvraj's resume PDF. Call this whenever the user asks about Yuvraj, his background, skills, experience, or wants to see his resume." } },
  { type: "function", function: { name: "open_launchpad", description: "Opens the Launchpad overlay to show all system apps." } },
...
export default function Siri({ closeSiri }: { closeSiri?: () => void }) {
  ...
  const downloadResume = useCallback(() => {
    ...
    link.download = "Yuvraj_Sen_Resume.pdf";
    link.target = "_blank";
  ...
  const executeTool = useCallback(async (name: string, args: any): Promise<string> => {
    ...
    switch (name) {
      ...
      case "download_resume":
        downloadResume();
        return "Here's Yuvraj's resume! He's a super talented backend developer. The download should start right up!";
      ...
      case "open_launchpad": {
        ...
        return "Opening up the Launchpad! Check out all the apps.";
      }
  ...
  const speakText = useCallback((text: string) => {
    ...
    // Check if we have pre-recorded voice audio files
    let audioSrc: string | null = null;
    if (text === SIRI_FALLBACK || text.includes("I can only perform actions that Yuvraj has set up") || text.includes("curiosity")) {
      audioSrc = "/music/failsafe.mp3";
    } else if (text.includes("Playing music now")) {
      audioSrc = "/music/playsong.mp3";
    } else if (text.includes("Here's Yuvraj's resume") || text.includes("Yuvraj Sen")) {
      audioSrc = "/music/yuvrajintro.mp3";
    }
    ...
    if (audioSrc) {
      ...
      audio.onerror = () => {
        fallbackSpeak(text, onAudioEnd);
      };
      audio.play().catch(() => fallbackSpeak(text, onAudioEnd));
      return;
    }

    fallbackSpeak(text, onAudioEnd);
  }, [closeSiri, closeAppById]);
