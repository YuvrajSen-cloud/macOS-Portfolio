...
export default function Launchpad({ show, toggleLaunchpad }: LaunchpadProps) {
  ...
  const search = () => {
    const systemApps = apps.filter(app => app.id !== "launchpad");
    let appsToRender = systemApps;
    if (isMobile) {
      appsToRender = systemApps.filter(app => !app.hideOnMobile).map(app => ({
        ...app,
  ...
  const handleAppClick = (e: React.MouseEvent, app: any) => {
    ...
    if (app.link) {
      setSafariUrl(app.link);
      window.dispatchEvent(new CustomEvent("launchpad:openSafari"));
    } else {
      window.dispatchEvent(new CustomEvent("launchpad:openApp", { detail: app.id }));
    }
  };
