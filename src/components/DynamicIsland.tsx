...
export default function DynamicIsland({ currentApp }: DynamicIslandProps) {
  ...
  const { audioState, controls, currentTrack } = useAudioContext();
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ color: "white", fontSize: 13, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {currentTrack?.title || music.title}
                      </div>
                      <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 11 }}>{currentTrack?.artist || music.artist}</div>
                    </div>
